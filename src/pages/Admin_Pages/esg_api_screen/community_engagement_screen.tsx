...
import {
    ...
    type CommunityEngagementResponse,
    type CommunityEngagementParams,
...
const CommunityEngagementScreen = () => {
    ...
    const fetchCommunityEngagementData = async () => {
        ...
        try {
            ...
            if (selectedCompany) {
                ...
                if (years.length > 0) {
                    ...
                    const params: CommunityEngagementParams = {
                        companyId: selectedCompanyId,
                        year: yearToFetch,
                    };

                    const data = await getCommunityEngagementData(params);
                    ...
                    const currentYear = new Date().getFullYear();
                    const data = await getCommunityEngagementData({
                        companyId: selectedCompanyId,
                        year: currentYear,
                    });
                    setCommunityData(data);
            ...
            setError(err.message || "Failed to fetch community engagement data");
            console.error("Error fetching community engagement data:", err);
        } finally {
            setLoading(false);
    ...
    const handleCompanyChange = (companyId: string) => {
        ...
        navigate(`/admin_community_engagement/${companyId}`);
    };
    ...
    const handleMetricClick = (metric: any, modalType: string) => {
        console.log("Community engagement metric clicked:", metric, modalType);
    };

    // Handle calculation click
    const handleCalculationClick = (calculationType: string, data?: any) => {
        console.log("Community engagement calculation clicked:", calculationType, data);
    };

    // Calculate summary metrics for community engagement data
    const summaryMetrics = useMemo(() => {
        if (!communityData) return null;

        const engagementSummary = getCommunityEngagementSummary(communityData.data);
        const socialLicense = getSocialLicenseDetails(communityData.data);
        ...
        const kpis = getKPIs(communityData.data);
        ...
        return {
            socialLicenseScore: engagementSummary.socialLicenseScore,
            overallEngagementScore: engagementSummary.overallEngagementScore,
            sdgAlignmentScore: engagementSummary.sdgAlignmentScore,
            communityTrustIndex: engagementSummary.communityTrustIndex,
            stakeholderSatisfaction: engagementSummary.stakeholderSatisfaction,
            socialLicenseLevel: socialLicense.level,
            socialLicenseFactors: socialLicense.factors,
            socialLicenseRecommendations: socialLicense.recommendations,
            prioritySDGs: sdgAlignment.prioritySDGs,
            alignmentStrengths: sdgAlignment.alignmentStrengths,
            educationBenefits: communityBenefits.education,
            healthcareBenefits: communityBenefits.healthcare,
            economicBenefits: communityBenefits.economic,
            environmentalBenefits: communityBenefits.environmental,
            engagementByCategory: engagementScores.byCategory,
            engagementTrend: engagementScores.trend,
            yearOverYearChange: engagementScores.yearOverYearChange,
            communityInvestmentRatio: kpis.community_investment_ratio,
            socialROIMultiplier: kpis.social_roi_multiplier,
            programEfficiency: kpis.program_efficiency,
            sustainabilityIndex: kpis.sustainability_index,
            strategicStrengths: strategicInsights.strengths,
            strategicOpportunities: strategicInsights.opportunities,
            strategicRisks: strategicInsights.risks,
            isImproving: trendAnalysis.isImproving,
            needsAttention: trendAnalysis.needsAttention,
            totalLocalProcurement: impactSummary.totalLocalProcurement,
            estimatedJobsCreated: impactSummary.estimatedJobsCreated,
            studentsImpacted: impactSummary.studentsImpacted,
            patientsServed: impactSummary.patientsServed,
        };
    }, [communityData]);

    // Get coordinates from community data
    const getCoordinates = () => {
        if (!communityData) return [];
        return communityData.data.company.area_of_interest_metadata?.coordinates || [];
    };

    // Get area name and coverage from community data
    const getAreaInfo = () => {
        if (!communityData) return { name: "", covered: "" };
        const metadata = communityData.data.company.area_of_interest_metadata;
        return {
            name: metadata?.name || "Community Engagement Area",
            covered: metadata?.area_covered || "N/A"
        };
    };

    // Get all community engagement graphs
    const getCommunityGraphs = () => {
        if (!communityData) return [];
