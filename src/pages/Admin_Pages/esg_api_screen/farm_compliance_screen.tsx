...
const FarmComplianceScreen = () => {
    ...
    // Fetch farm compliance data
    const fetchFarmComplianceData = async () => {
        ...
        try {
            ...
            if (selectedCompany) {
                ...
                if (years.length > 0) {
                    ...
                    const params: FarmComplianceParams = {
                        companyId: selectedCompanyId,
                        year: yearToFetch,
                    };

                    const data = await getFarmComplianceData(params);
                    ...
                    const currentYear = new Date().getFullYear();
                    const data = await getFarmComplianceData({
                        companyId: selectedCompanyId,
                        year: currentYear,
                    });
                    setComplianceData(data);
            ...
            setError(err.message || "Failed to fetch farm compliance data");
            console.error("Error fetching farm compliance data:", err);
        } finally {
            setLoading(false);
    ...
    const handleCompanyChange = (companyId: string) => {
        ...
        navigate(`/admin_farm_compliance/${companyId}`);
    };
    ...
    // Fetch farm compliance data when company is selected and companies are loaded
    useEffect(() => {
                        ...
                        <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-lg">
                            <div className="flex items-center gap-3 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-600">Choose a company to view Farm Management Compliance (Training + Scope 3) Data</p>
                                </div>
                            </div>
    ...
    // Loading state for compliance data (only when company selected)
    if (loading) {
        return (
            <div className="flex min-h-screen bg-gray-50 text-gray-900">
                <Sidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-lg sm:text-xl font-bold" style={{ color: DARK_GREEN }}>
                                        Farm Management Compliance (Training + Scope 3)
                                    </h1>
                                    {selectedCompany && (
...
export default FarmComplianceScreen;