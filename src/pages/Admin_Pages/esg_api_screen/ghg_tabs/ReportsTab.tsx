...
// Icons – identical set to keep look & feel
import {
    FileText,
    Download,
    Printer,
...
import {
    ...
    getDataAvailability,
    getReportingYear,
    // getVerificationStatus as getGhgVerificationStatus,
...
// Enhanced Color Palette – keep exact same green theme as ReportsTab
const COLORS = {
    primary: '#22c55e',
    primaryDark: '#16a34a',
    primaryLight: '#86efac',
...
interface GHGReportsTabProps {
    ...
    getTrendIcon: (trend: string) => JSX.Element; // optional, not heavily used in reports
...
const GHGReportsTab = ({
    ...
    // State – identical to soil carbon ReportsTab
    ...
    // ------------------------------------------------------------------------
    const metadata = ghgData ? getMetadata(ghgData) : null;
    ...
    // const verificationStatus = ghgData ? getGhgVerificationStatus(ghgData) : null; // from data_availability.carbon_data_quality
    ...
    const environmentalMetrics = ghgData ? getEnvironmentalMetricsSummary(ghgData) : [];
    ...
    // Derived values for summary cards / confidence
    const completenessScore = dataAvailability?.carbon_data_quality?.completeness_score || 0;
    const verificationStatusText = dataAvailability?.carbon_data_quality?.verification_status || 'Pending';
    const verificationYears = carbonAccounting?.yearly_data?.length || 0;
    const totalYears = availableYears.length || 1;

    // Carbon credit readiness – not directly in GHG service; we derive from completeness & verification
    const carbonCreditReadiness = {
        status: completenessScore > 80 && verificationStatusText === 'verified' ? 'Ready' : 'Not Ready',
        requirements_met: completenessScore > 80 ? 8 : 4,
        total_requirements: 10,
        requirements_met_percent: completenessScore > 80 ? 80 : 40,
    };

    // Confidence score breakdown – derived from available metrics
    const confidenceBreakdown = [
        {
            label: 'Data Completeness',
            value: completenessScore,
            formatted: `${completenessScore}%`,
        },
        {
            label: 'Verification Status',
            value: verificationStatusText === 'verified' ? 95 : 70,
            formatted: verificationStatusText,
        },
        {
        ...
        },
        {
            ...
            value: sequestrationMethodologies.length > 0 ? 88 : 60,
            formatted: sequestrationMethodologies.length > 0 ? 'Robust' : 'Basic',
        },
    ];
    ...
    // ------------------------------------------------------------------------
    // Prepare report data – follows exact structure of soil ReportsTab
    // ------------------------------------------------------------------------
    const prepareReportData = () => {
        // Compliance Checklist Data (GHG specific)
        const complianceChecklist = [
            {
                requirement: 'GHG Protocol (Corporate Standard)',
                status: verificationStatusText === 'verified' ? 'Completed' : 'In Progress',
                // date: metadata?.generated_at
                //     ? new Date(metadata.generated_at).toLocaleDateString()
                //     : 'N/A',
                // verifiedBy: verificationStatus?.verified_by || 'System',
            },
            {
                requirement: 'ISO 14064-1:2018',
                status: completenessScore > 70 ? 'In Progress' : 'Pending',
                date: reportingYear?.toString() || 'N/A',
                verifiedBy: 'System',
            },
            {
                ...
                date: metadata?.generated_at
                    ? new Date(metadata.generated_at).toLocaleDateString()
                    : 'N/A',
                verifiedBy: carbonCreditReadiness.status === 'Ready' ? 'Verra' : 'N/A',
            },
        ];

        // Emission Factors Table Data
        const emissionFactorsData = emissionFactors.slice(0, 10).map((factor: any) => ({
            id: factor._id || 'N/A',
            ...
            isActive: factor.is_active !== undefined ? factor.is_active : true,
        }));

        // Methodology Data
        const methodologyData = {
            selectedYear: selectedYear || reportingYear || new Date().getFullYear(),
            calculationMethods: companyInfo?.esg_reporting_framework?.length || 0,
            ...
            esgFrameworks: companyInfo?.esg_reporting_framework || [],
            carbonFramework: carbonAccounting?.framework || {},
            carbonMethodology: carbonAccounting?.methodology || 'N/A',
            scopeInfo: {
                scope1: keyMetrics?.total_ghg_emissions?.scope1 || 0,
                scope2: keyMetrics?.total_ghg_emissions?.scope2 || 0,
                scope3: keyMetrics?.total_ghg_emissions?.scope3 || 0,
            },
            environmentalMetricsCount: environmentalMetrics.length,
            environmentalCategories: environmentalMetrics.reduce((acc: any, metric: any) => {
    ...
    // ------------------------------------------------------------------------
    // Table columns – identical to soil ReportsTab (only data changes)
    // ------------------------------------------------------------------------
    const tableColumns = {
        compliance: [
            { key: 'requirement', header: 'Requirement', className: 'font-semibold' },
            {
                ...
                accessor: (row: any) => (
                    <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${row.status === 'Completed'
        ...
        emissionFactors: [
            { key: 'source', header: 'Source' },
            {
                key: 'value',
                header: 'Value',
                accessor: (row: any) => formatNumber(row.value),
            },
            { key: 'unit', header: 'Unit' },
            {
                key: 'gwp',
                header: 'GWP',
                accessor: (row: any) => row.gwp.toFixed(2),
            },
    ...
    // ------------------------------------------------------------------------
    // Export handler – same as original
    // ------------------------------------------------------------------------
    const handleExport = () => {
    ...
    // ------------------------------------------------------------------------
    // Report Summary Cards – GHG specific content, same green gradient theme
    // ------------------------------------------------------------------------
    const summaryCards = [
        {
            title: 'Verification Status',
            value: verificationStatusText,
            description: `${verificationYears} of ${totalYears} years verified`,
            icon: <ShieldCheck className="w-8 h-8 text-white" />,
            color: 'from-green-500 to-emerald-600',
        },
        {
            title: 'Carbon Credit Readiness',
            value: `${carbonCreditReadiness.requirements_met_percent}%`,
            description: `${carbonCreditReadiness.requirements_met} of ${carbonCreditReadiness.total_requirements} requirements met`,
            icon: <Target className="w-8 h-8 text-white" />,
            color: 'from-yellow-500 to-amber-600',
        },
        {
            title: 'Data Quality',
            value: completenessScore > 80 ? 'High' : completenessScore > 60 ? 'Medium' : 'Low',
            description: `${completenessScore}% completeness`,
    ...
    // ------------------------------------------------------------------------
    // Render – exact same JSX structure, only content placeholders replaced
    // ------------------------------------------------------------------------
    return (
        <div className="space-y-8 pb-8">
            {/* Report Type Navigation – identical tabs */}
                        ...
                        <div className="space-y-4">
                            <div className="p-5 rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-gray-900 mb-1">Emission Reduction Status</h4>
                                        <p className="text-gray-700">
                                            Total emissions:{' '}
                                            {formatNumber(keyMetrics?.total_ghg_emissions?.value || 0)}{' '}
                                            {keyMetrics?.total_ghg_emissions?.unit || 'tCO₂e'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-5 rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200">
                                <div className="flex items-start gap-3">
                                    <CheckCircle className="w-6 h-6 text-green-600 mt-0.5 flex-shrink-0" />
                                    <div>
                                        <h4 className="font-semibold text-gray-900 mb-1">Net Carbon Balance</h4>
                                        <p className="text-gray-700">
                                            {keyMetrics?.net_carbon_balance?.value &&
                                                keyMetrics.net_carbon_balance.value < 0
                                                ? 'Carbon Negative (Net Sink)'
                                                : 'Carbon Positive (Net Source)'}
                                            : {formatNumber(keyMetrics?.net_carbon_balance?.value || 0)}{' '}
                                            {keyMetrics?.net_carbon_balance?.unit || 'tCO₂e'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-5 rounded-2xl bg-gradient-to-br from-yellow-50 to-amber-50 border-2 border-yellow-200">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-gray-700">
                                            {verificationStatusText} verification for {verificationYears} of{' '}
                                            {totalYears} years
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div className="p-5 rounded-2xl bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-gray-700">
                                            {carbonCreditReadiness.requirements_met} of{' '}
                                            {carbonCreditReadiness.total_requirements} compliance requirements met
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div className="p-6 rounded-2xl border-2 border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
                            ...
                            <p className="text-sm text-gray-600">Data analyzed for this reporting period</p>
                        </div>
                        ...
                        <div className="p-6 rounded-2xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-cyan-50">
                            ...
                            <p className="text-sm text-gray-600">Different calculation approaches used</p>
                        </div>
                        ...
                        <div className="p-6 rounded-2xl border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-pink-50">
                            ...
                            <p className="text-sm text-gray-600">Primary and secondary data sources</p>
                        </div>
                    </div>
                    ...
                    {/* ESG Frameworks & Carbon Accounting */}
                    ...
                    {/* Environmental Metrics & Data Sources */}
                    <div className="bg-white rounded-3xl border border-gray-200 shadow-lg p-8">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {reportData.methodologyData.dataSources.length > 0 ? (
                                    reportData.methodologyData.dataSources.map((source: string, index: number) => (
                                        <span
                                            key={index}
                                            className="px-4 py-2 rounded-xl text-sm font-medium bg-gray-100 text-gray-800 border border-gray-200"
                                        >
                                            {source}
                                        </span>
                                    ))
            ...
            {/* -------------------- TECHNICAL DATA (Emission Factors) -------------------- */}
            ...
            {selectedReport === 'export' && (
                <div className="space-y-8">
                    <div className="bg-white rounded-3xl border border-gray-200 shadow-lg p-8 text-center py-16">
                        <Download className="w-16 h-16 text-green-600 mx-auto mb-4" />
                        <h3 className="text-2xl font-bold text-gray-900 mb-2">Export GHG Report</h3>
                        <p className="text-gray-600 mb-8 max-w-md mx-auto">
                            Generate a comprehensive report containing all GHG emissions data, methodology, compliance
                            status, and emission factors.
                        </p>
                        <button
                        ...
                        <div className="p-8 space-y-8">
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    ...
                                    >
                                        <FileText
                                            className={`w-10 h-10 mx-auto mb-3 ${exportFormat === 'pdf' ? 'text-green-600' : 'text-gray-400'
                                                }`}
                                        />
                                        <span
                                            className={`font-semibold ${exportFormat === 'pdf' ? 'text-green-700' : 'text-gray-700'
                                                }`}
                                        >
                                            PDF
                                        </span>
                                    ...
                                    >
                                        <FileSpreadsheet
                                            className={`w-10 h-10 mx-auto mb-3 ${exportFormat === 'excel' ? 'text-green-600' : 'text-gray-400'
                                                }`}
                                        />
                                        <span
                                            className={`font-semibold ${exportFormat === 'excel' ? 'text-green-700' : 'text-gray-700'
                                                }`}
                                        >
                                            Excel
                                        </span>
                                    ...
                                    >
                                        <FileText
                                            className={`w-10 h-10 mx-auto mb-3 ${exportFormat === 'csv' ? 'text-green-600' : 'text-gray-400'
                                                }`}
                                        />
                                        <span
                                            className={`font-semibold ${exportFormat === 'csv' ? 'text-green-700' : 'text-gray-700'
                                                }`}
                                        >
                                            CSV
                                        </span>
            ...
            {/* Audit Log Details Modal – preserved from original (commented out in soil) */}
            {selectedAuditLog && (
                <div
                    className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
                ...
                >
                    <div
                        className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-t-3xl">
                            <div className="flex items-center justify-between">
                        ...
                        <div className="p-8 space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-5 rounded-xl bg-gray-50">
                                    ...
                                    <span
                                        className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${selectedAuditLog.status === 'Completed'
                                                ? 'bg-green-100 text-green-800'
                                                : selectedAuditLog.status === 'In Progress'
                                                    ? 'bg-yellow-100 text-yellow-800'
                                                    : 'bg-gray-100 text-gray-800'
                                            }`}
                                    >
...
// ------------------------------------------------------------------------
// Missing icon components – copied verbatim from original ReportsTab
// ------------------------------------------------------------------------
