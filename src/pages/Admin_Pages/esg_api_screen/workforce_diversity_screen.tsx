...
import {
    ...
    type WorkforceDiversityResponse,
    type WorkforceDiversityParams,
    getWorkforceSummary,
...
const WorkforceDiversityScreen = () => {
    ...
    // Fetch workforce diversity data
    const fetchWorkforceDiversityData = async () => {
        ...
        try {
            ...
            if (selectedCompany) {
                ...
                if (years.length > 0) {
                    ...
                    const params: WorkforceDiversityParams = {
                        companyId: selectedCompanyId,
                        year: yearToFetch,
                    };

                    const data = await getWorkforceDiversityData(params);
                    ...
                    const currentYear = new Date().getFullYear();
                    const data = await getWorkforceDiversityData({
                        companyId: selectedCompanyId,
                        year: currentYear,
                    });
                    setWorkforceData(data);
            ...
            setError(err.message || "Failed to fetch workforce diversity data");
            console.error("Error fetching workforce diversity data:", err);
        } finally {
            setLoading(false);
    ...
    const handleCompanyChange = (companyId: string) => {
        ...
        navigate(`/admin_workforce_diversity/${companyId}`);
    };
    ...
    const handleMetricClick = (metric: any, modalType: string) => {
        console.log("Workforce metric clicked:", metric, modalType);
    };

    // Handle calculation click
    const handleCalculationClick = (calculationType: string, data?: any) => {
        console.log("Workforce calculation clicked:", calculationType, data);
    };

    // Calculate summary metrics for workforce data
    const summaryMetrics = useMemo(() => {
        ...
        return {
            totalEmployees: metricsSummary.totalEmployees,
            genderDiversity: metricsSummary.genderDiversity,
            trainingHours: metricsSummary.trainingHours,
            engagementScore: metricsSummary.engagementScore,
            maleEmployees: metricsSummary.maleEmployees,
            femaleEmployees: metricsSummary.femaleEmployees,
            permanentEmployees: metricsSummary.permanentEmployees,
            fixedTermEmployees: metricsSummary.fixedTermEmployees,
            leadershipDiversity: metricsSummary.leadershipDiversity,
            payEquity: metricsSummary.payEquity,
            retentionRate: metricsSummary.retentionRate,
            inclusionScore: metricsSummary.inclusionScore,
            keyIndicators: metricsSummary.keyIndicators,
            contractTypes: composition.contract_types,
            trainingSummary: composition.training_summary,
            socialMetrics: socialMetrics.metrics,
            recommendations: workforceData.data.recommendations || [],
        };
    }, [workforceData]);

    // Get coordinates from workforce data
    const getCoordinates = () => {
        if (!workforceData) return [];
        return workforceData.data.company.area_of_interest_metadata?.coordinates || [];
    };

    // Get area name and coverage from workforce data
    const getAreaInfo = () => {
        if (!workforceData) return { name: "", covered: "" };
        const metadata = workforceData.data.company.area_of_interest_metadata;
        return {
            name: metadata?.name || "Workforce Area",
            covered: metadata?.area_covered || "N/A"
        };
    };

    // Get all workforce graphs
    const getWorkforceGraphs = () => {
        if (!workforceData) return [];
    ...
    const sharedData = {
        ...
        summaryMetrics,
        getWorkforceGraphs,
        getWorkforceSummary: () => workforceData ? getWorkforceSummary(workforceData) : null,
        getWorkforceComposition: () => workforceData ? getWorkforceComposition(workforceData) : null,
        getInclusionAndBelonging: () => workforceData ? getInclusionAndBelonging(workforceData) : null,
        getSocialMetrics: () => workforceData ? getSocialMetrics(workforceData) : null,
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-lg sm:text-xl font-bold" style={{ color: DARK_GREEN }}>
                                        Workforce Diversity & Inclusion Dashboard
                                    </h1>
                                    {selectedCompany && (
...
export default WorkforceDiversityScreen;