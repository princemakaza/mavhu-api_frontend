...
import {
    ...
    type WasteManagementResponse,
    type WasteManagementParams,
    getWasteSummary,
    getWasteStreams,
...
} from "../../../services/Admin_Service/esg_apis/waste_management_service";

// Import tab components (you'll need to create these)
import WasteOverviewTab from "./waste_tabs/WasteOverviewTab";
import WasteAnalyticsTab from "./waste_tabs/WasteAnalyticsTab";
import WasteReportsTab from "./waste_tabs/WasteReportsTab";

// Color Palette (same as Energy Consumption Screen)
...
const WasteManagementScreen = () => {
    ...
    const [wasteData, setWasteData] = useState<WasteManagementResponse | null>(null);
        ...
        try {
            ...
            if (selectedCompany) {
                ...
                if (years.length > 0) {
                    ...
                    const params: WasteManagementParams = {
                        companyId: selectedCompanyId,
                        year: yearToFetch,
                    };

                    const data = await getWasteManagementData(params);
                    setWasteData(data);
                    ...
                    const currentYear = new Date().getFullYear();
                    const data = await getWasteManagementData({
                        companyId: selectedCompanyId,
                        year: currentYear,
                    });
                    setWasteData(data);
            ...
            setError(err.message || "Failed to fetch waste management data");
    ...
    const handleCompanyChange = (companyId: string) => {
        ...
        navigate(`/admin_waste_management/${companyId}`);
    };
    ...
    const handleMetricClick = (metric: any, modalType: string) => {
        console.log("Waste metric clicked:", metric, modalType);
    };

    // Handle calculation click
    const handleCalculationClick = (calculationType: string, data?: any) => {
        console.log("Waste calculation clicked:", calculationType, data);
    };

    // Calculate summary metrics for waste data
    const summaryMetrics = useMemo(() => {
        if (!wasteData) return null;

        const metricsSummary = getWasteMetricsSummary(wasteData);
        const incidents = getIncidentsAndTargets(wasteData);
        const circular = getCircularEconomy(wasteData);

        return {
            recyclingRate: metricsSummary.recyclingRate,
            hazardousWaste: metricsSummary.hazardousWaste,
            totalWaste: metricsSummary.totalWaste,
            wasteIncidents: metricsSummary.wasteIncidents,
            costSavings: metricsSummary.costSavings,
            totalWasteStreams: metricsSummary.totalWasteStreams,
            wasteStreams: metricsSummary.wasteStreams,
            currentYearIncidents: metricsSummary.currentYearIncidents,
            complianceStatus: metricsSummary.complianceStatus,
            materialsRecovered: metricsSummary.materialsRecovered,
            wasteToEnergy: metricsSummary.wasteToEnergy,
            closedLoopProjects: metricsSummary.closedLoopProjects,
            circularSupplyChain: metricsSummary.circularSupplyChain,
            zeroWasteTargets: incidents.zero_waste_targets?.targets || [],
            wasteIncidentsList: incidents.incidents || [],
            circularInitiatives: circular.initiatives || [],
        };
    }, [wasteData]);

    // Get coordinates from waste data
    const getCoordinates = () => {
        if (!wasteData) return [];
        return wasteData.data.company.area_of_interest_metadata?.coordinates || [];
    };

    // Get area name and coverage from waste data
    const getAreaInfo = () => {
        if (!wasteData) return { name: "", covered: "" };
        const metadata = wasteData.data.company.area_of_interest_metadata;
        return {
            name: metadata?.name || "Waste Management Area",
            covered: metadata?.area_covered || "N/A"
        };
    };

    // Get all waste graphs
    const getWasteGraphs = () => {
        if (!wasteData) return [];
        return getAllWasteGraphs(wasteData);
    };
    ...
    // Fetch waste management data when company is selected and companies are loaded
    useEffect(() => {
    ...
    const sharedData = {
        ...
        summaryMetrics,
        getWasteGraphs,
        getWasteSummary: () => wasteData ? getWasteSummary(wasteData) : null,
        getWasteStreams: () => wasteData ? getWasteStreams(wasteData) : null,
        getIncidentsAndTargets: () => wasteData ? getIncidentsAndTargets(wasteData) : null,
        getCircularEconomy: () => wasteData ? getCircularEconomy(wasteData) : null,
        getEnvironmentalMetrics: () => wasteData ? getEnvironmentalMetrics(wasteData) : null,
        getDetailedWasteMetrics: () => wasteData ? getDetailedWasteMetrics(wasteData) : null,
    };
                    ...
                    <div className="max-w-6xl mx-auto">
                        <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-lg">
                            <div className="flex items-center gap-3 mb-8">
                                <Trash2 className="w-10 h-10" style={{ color: PRIMARY_GREEN }} />
                                <div>
                                    <h1 className="text-3xl font-bold bg-gradient-to-r from-green-500 to-green-700 bg-clip-text text-transparent">
                                        Select Company
                                    </h1>
                                    <p className="text-gray-600">Choose a company to view Waste Management & Circular Economy Data</p>
                                </div>
                            </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="p-3 rounded-lg bg-green-50 border border-green-200 group-hover:bg-green-100 transition-colors">
                                                    <Trash2 className="w-6 h-6" style={{ color: PRIMARY_GREEN }} />
                                                </div>
    ...
    // Loading state for waste data (only when company selected)
    if (loading) {
        return (
            <div className="flex min-h-screen bg-gray-50 text-gray-900">
                <Sidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-lg sm:text-xl font-bold" style={{ color: DARK_GREEN }}>
                                        Waste Management & Circular Economy Dashboard
                                    </h1>
                                    {selectedCompany && (
                            ...
                            {[
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-4 py-1.5 rounded-lg font-medium whitespace-nowrap transition-all text-sm ${
                                            activeTab === tab.id
